...
async def main():
    async with aiohttp.ClientSession() as session:
        async with session.get('http://167.99.161.99:8080/status', headers={'auth': 'PASSWORD'}) as resp:
            global stats
            txt = await resp.text()
...
def strfix(str, tabin=1, prelen=0):
    ...
    str = str.replace("\n  ", tabstr)
    str = str.replace("\n", tabstr)
    str = str.replace(tabstr+tabstrr, tabstr)
    return str
def draw_menu2(stdscr):
    ...
    while (k != ord('q')):
        ...
        stdscr.addstr(3, tab*1, 'dnf: ')
        stdscr.addstr(9, tab*1, 'nano: ')
        stdscr.attroff(curses.A_BOLD)
        stdscr.attroff(curses.color_pair(2))
        stdscr.attron(curses.color_pair(1))
        stdscr.addstr(2, 7+tab*1, strfix(f'{pover["nginx"]}', 1, 7))
        stdscr.addstr(3, 5+tab*1, strfix(f'{pover["dnf"]}', 1, 5))
        stdscr.addstr(9, 6+tab*1, strfix(f'{pover["nano"]}', 1, 6))
