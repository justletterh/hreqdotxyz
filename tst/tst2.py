...
async def main():
    async with aiohttp.ClientSession() as session:
        async with session.get('http://142.93.69.231:8080/status', headers={'auth': '~&Ic7<^pg~QC&x|tdk5E@%dGqM&g<AMml6/^q.2.p?)f6agN1L9ZD{HIF,J:u?9S2E|,5BXf{X<'}) as resp:
            global stats
            txt = await resp.text()
...
def draw_menu2(stdscr):
    ...
    while (k != ord('q')):
        ...
        stdscr.addstr(20, tab*1, 'dotnet:')
        stdscr.attroff(curses.A_UNDERLINE)
        stdscr.addstr(2, tab*1, 'ruby: ')
        stdscr.addstr(3, tab*1, 'julia: ')
        stdscr.addstr(4, tab*1, 'php: ')
        stdscr.addstr(8, tab*1, 'go: ')
        stdscr.addstr(9, tab*1, 'js: ')
        stdscr.addstr(10, tab*1, 'lua: ')
        stdscr.addstr(15, tab*1, 'rust: ')
        stdscr.addstr(16, tab*1, 'crystal: ')
        stdscr.addstr(21, tab*2, 'version: ')
        stdscr.addstr(22, tab*2, 'runtmes: ')
        stdscr.addstr(27, tab*2, 'sdks: ')
        stdscr.addstr(29, tab*1, 'dart: ')
        stdscr.addstr(30, tab*1, 'elixir: ')
        stdscr.addstr(33, tab*1, 'nginx: ')
        stdscr.addstr(34, tab*1, 'docker: ')
        stdscr.addstr(35, tab*1, 'docker-compose: ')
        stdscr.addstr(36, tab*1, 'apt: ')
        stdscr.addstr(37, tab*1, 'nano: ')
        stdscr.attroff(curses.A_BOLD)
        stdscr.attroff(curses.color_pair(2))
        stdscr.attron(curses.color_pair(1))
        stdscr.addstr(2, 6+tab*1, strfix(f'{pover["ruby"]}', 1, 6))
        stdscr.addstr(3, 7+tab*1, strfix(f'{pover["julia"]}', 1, 7))
        stdscr.addstr(4, 5+tab*1, strfix(f'{pover["php"]}', 1, 5))
        stdscr.addstr(8, 4+tab*1, strfix(f'{pover["go"]}', 1, 4))
        stdscr.addstr(9, 4+tab*1, strfix(f'{pover["js"]}', 1, 4))
        stdscr.addstr(10, 5+tab*1, strfix(f'{pover["lua"]}', 1, 5))
        stdscr.addstr(15, 6+tab*1, strfix(f'{pover["rust"]}', 1, 6))
        stdscr.addstr(16, 9+tab*1, strfix(f'{pover["crystal"]}', 1, 9))
        stdscr.addstr(21, 9+tab*2, strfix(f'{pdnet["ver"]}', 2, 9))
        stdscr.addstr(22, 9+tab*2, strfix(f'{pdnet["runtimes"]}', 2, 9))
        stdscr.addstr(27, 6+tab*2, strfix(f'{pdnet["sdks"]}', 2, 6))
        stdscr.addstr(29, 6+tab*1, strfix(f'{pover["dart"]}', 1, 6))
        stdscr.addstr(30, 8+tab*1, strfix(f'{pover["elixir"]}', 1, 8))
        stdscr.addstr(33, 7+tab*1, strfix(f'{pover["nginx"]}', 1, 7))
        stdscr.addstr(34, 8+tab*1, strfix(f'{pover["docker"]}', 1, 8))
        stdscr.addstr(35, 16+tab*1, strfix(f'{pover["docker-compose"]}', 1, 16))
        stdscr.addstr(36, 5+tab*1, strfix(f'{pover["apt"]}', 1, 5))
        stdscr.addstr(37, 6+tab*1, strfix(f'{pover["nano"]}', 1, 6))
